...
#include "myRTClib.h"
 ...
 #define WIRE Wire
#else
//#elif defined(ESP8266)// beide regels toegevoegd ipv '#else'
 //#include <pgmspace.h>//
 #define PROGMEM
 ...
 *   Day of year                                *
 ************************************************/
//http://hack.lenotta.com/arduino-diy-stupid-plants-irrigator/
uint16_t DateTime::dayOfYear() const {
uint16_t days=d;


for (uint8_t i=1; i < m; ++i){
days+=pgm_read_byte(daysInMonth +i-1); // tel de maanddagen uit PROGMEM-array bij elkaar op

if(m> 2 && yOff % 4==0)// indien  na februari en schrikkeljaar: verhoog met 1
{
days=days+1;
}
}
